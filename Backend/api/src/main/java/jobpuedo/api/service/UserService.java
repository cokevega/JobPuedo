...
public class UserService {
	...
	public List<User> filterUsers(UserSearchedRequest userSearched) {
		...
		ExampleMatcher matcher = ExampleMatcher.matching();// Filtro de coincidencias para tipo String
	...
	public void deleteUserDefinitely(User user) {
		// Eliminar usuario definitivamente: eliminar primero registros huérfanos
		user.setRoles(null);
		repo.save(user);
	...
	public Collection<User> searchWorkers(WorkerSearchedRequest worker) {
		// Filtrado manual de trabajadores
		...
		if (!worker.getEducation().isBlank()) {
			// Se buscan formaciones y se elabora una colección de usuarios Set (sin valores
			// repetidos)
			...
			//Si no se necesita filtrado por experiencia, simplemente se retorna este resultado
			if(worker.getExperiences().isBlank()) return usersEducation;
		}
		if (!worker.getExperiences().isBlank()) {
			// Se elabora otro Set con experiencias, pero ahora juntando dos Set, porque se
			// evalúan dos campos
			// Campo position
		...
		// Se juntan los valores coincidentes de ambos Set (experiencas laborales y
		// formación)
